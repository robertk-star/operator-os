...
export function SequenceStudio({
  ...
  async function copySequence(sequence: Sequence) {
    ...
    setMessage(`Copied into a new draft. Edit it, then activate when ready.`);
  ...
  return (
    <div className="stack wide">
      ...
      <div className="card stack">
          ...
          <div className="row">
            {mailboxes.map((account) => (
              <label key={account.id} className="chip">
                <input type="checkbox" checked={mailboxIds.includes(String(account.id))} onChange={() => setMailboxIds((current) => (current.includes(String(account.id)) ? current.filter((item) => item !== String(account.id)) : [...current, String(account.id)]))} />
                {account.email}
              </label>
            ))}
          </div>
          {!mailboxes.length ? <p className="meta">No mailboxes synced. Use Admin to sync mailboxes.</p> : null}
        </div>
        <div>
          <p className="meta">Sending days</p>
          <div className="row">
            {DAYS.map((day) => (
              <button key={day.id} type="button" className={sendingDays.includes(day.id) ? "kpi-dark chip" : "chip"} onClick={() => setSendingDays((current) => (current.includes(day.id) ? current.filter((item) => item !== day.id) : [...current, day.id].sort()))}>
                {day.label}
              </button>
            ))}
          </div>
        ...
        {draftSteps.map((step, index) => (
          <div key={index} className="card stack">
            <div className="row" style={{ justifyContent: "space-between" }}>
              ...
              {draftSteps.length > 1 ? <button type="button" className="chip" onClick={() => setDraftSteps((current) => current.filter((_, row) => row !== index))}>Remove</button> : null}
            </div>
            <div className="form-grid">
        ...
        <div className="row">
          <button type="button" onClick={() => void saveDraft()} style={{ background: "#17243f", color: "#fff", border: 0 }}>Save draft</button>
          <button type="button" className="chip" onClick={() => setPreview({ total: people.filter((item) => item.email).length, eligible: eligible.length })}>Preview audience</button>
          <button type="button" className="chip" onClick={() => void enrollEligible()}>Enroll eligible contacts</button>
          <button type="button" disabled={busy || !sequenceId} onClick={() => void control(sequenceId, "activate")}>{busy ? "Publishing..." : "Activate in Smartlead"}</button>
        </div>
        {preview ? <p className="meta">{preview.eligible} eligible of {preview.total} people with emails.</p> : null}
        {message ? <p className="meta">{message}</p> : null}
      </div>
      <SavedSequences sequences={sequences} people={people} queued={queued} busy={busy} onEdit={(sequence) => loadSequence(sequence as Sequence)} onCopy={(sequence) => void copySequence(sequence as Sequence)} onControl={(id, action) => void control(id, action)} />
      <div className="card stack">
